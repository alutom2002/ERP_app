...
describe("ERP", function () {
    ...
    it("change PO ACCEPTED", async function () {
        await contract.changeOrderStatus(1, 1);
    });
    it("change PO DELIVERING", async function () {
        await contract.changeOrderStatus(1, 3);
    });
    ...
    it("change PB PREPARE_MAT", async function () {
        const a = await contract.changePBStatus(1, 1);
    });
    it("change PB PRODUCE", async function () {
        const a = await contract.changePBStatus(1, 2);
    });
    it("finish PRODUCE change PB to DONE", async function () {
        const a = await contract.finishProduce(1);
    });
    it("change PB DONE", async function () {
        const a = await contract.changePBStatus(1, 4);
    });
    it("create SO", async function () {
        await contract.createSO("manager", "retailer", [[[[1, "Nike", "giay", 0, 1, 1]], 5]], 1231231);
    });
